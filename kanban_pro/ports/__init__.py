...
class KanbanBackend(Protocol):
    ...
    # them too (recovery/purge targets, migration).
    async def list_cards(self, board_id: str, include_archived: bool = False) -> list[Card]: ...
    async def get_card(self, card_id: str) -> Card: ...
    # card.placements must have >=1 entry (where to create it). An id already in the store
    # is a Conflict unless overwrite=True (migration re-runs: an import IS a re-import) —
    # so a card can never be clobbered by accident, e.g. by a colliding short random id.
    # An EMPTY card.id means "store, assign one" (the seq: id scheme; see domain.ids).
